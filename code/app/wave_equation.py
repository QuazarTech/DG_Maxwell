...
def Li_Lp_xi(L_xi_i, L_xi_p):
	...
	#LaTex strings
	#-------------
	#To calculate $L_{p}(\xi) L_{i}(\xi)$ for N Lagrange-Gaussian-Lobatto points in a $\xi$ space [-1, 1] and indices i and p ranging from 0 to N - 1, We need to start by creating a 1 X N x N matrix of $L_{p}(\xi)$.
...
#Now, we need to get a $L_{i}(\xi)$ matrix which is the transpose of $L_{p}(\xi)$ and reordered as \newline(1, 0, 2), where (0, 1, 2) is the normal ordering (x, y, z)
...
#Now, the $L_{i}(\xi)$ array is broadcasted across the dimension 0 N times. The result is an\newline N X N X N matrix with elements repeating N times in dimension 0.
...
#Now doing the same for $L_{i}(\xi)$, i.e., broadcasting it in dimension 1 N times. Another \newline N X N X N array would be obtained which can be multipled with the previous N X N X N array which would give the required $L_{p}(\xi) L_{i}(\xi)$ matrix
...
#\begin{tikzpicture}[every node/.style={anchor=north east,fill=white,minimum width=1.4cm,minimum height=10mm}]
#\matrix (mA) [draw,matrix of math nodes]
#{
#L_{0}(\xi_{N - 1})L_{0}(\xi_{N - 1}) & L_{0}(\xi_{N - 1})L_{1}(\xi_{N - 1}) & ... & L_{0}(\xi_{N - 1})L_{N - 1}(\xi_{N - 1}) \\
#L_{1}(\xi_{N - 1})L_{0}(\xi_{N - 1}) & L_{1}(\xi_{N - 1})L_{1}(\xi_{N - 1}) & ... & L_{1}(\xi_{N - 1})L_{N - 1}(\xi_{N - 1}) \\
#. & . & ... & . \\
#L_{N - 1}(\xi_{N - 1})L_{0}(\xi_{N - 1}) & L_{N - 1}(\xi_{N - 1})L_{1}(\xi_{N - 1}) & ... & L_{N - 1}(\xi_{N - 1})L_{N - 1}(\xi_{N - 1}) \\
#};

#\matrix (mB) [draw,matrix of math nodes] at ($(mA.south west)+(1.5,0.7)$)
#{
#L_{0}(\xi_{i})L_{0}(\xi_{i}) & L_{0}(\xi_{i})L_{1}(\xi_{i}) & ... & L_{0}(\xi_{i})L_{N - 1}(\xi_{i}) \\
#L_{1}(\xi_{i})L_{0}(\xi_{i}) & L_{1}(\xi_{i})L_{1}(\xi_{i}) & ... & L_{1}(\xi_{i})L_{N - 1}(\xi_{i}) \\
#. & . & ... & . \\
#L_{N - 1}(\xi_{i})L_{i}(\xi_{i}) & L_{N - 1}(\xi_{i})L_{1}(\xi_{i}) & ... & L_{N - 1}(\xi_{i})L_{N - 1}(\xi_{i}) \\
#};

#\matrix (mC) [draw,matrix of math nodes] at ($(mB.south west)+(1.5,0.7)$)
#{
#L_{0}(\xi_{0})L_{0}(\xi_{0}) & L_{0}(\xi_{0})L_{1}(\xi_{0}) & ... & L_{0}(\xi_{0})L_{N - 1}(\xi_{0}) \\
#L_{1}(\xi_{0})L_{0}(\xi_{0}) & L_{1}(\xi_{0})L_{1}(\xi_{0}) & ... & L_{1}(\xi_{0})L_{N - 1}(\xi_{0}) \\
#. & . & ... & . \\
#L_{N - 1}(\xi_{0})L_{0}(\xi_{0}) & L_{N - 1}(\xi_{0})L_{1}(\xi_{0}) & ... & L_{N - 1}(\xi_{0})L_{N - 1}(\xi_{0}) \\
#};
